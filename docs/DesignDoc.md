...
*  **Simulated Disk I/O:** Reading from "Disk" (during a Page Fault) is simulated by adding a time penalty (cycle count), not by reading actual files on the hard drive.
*  **Volatile Storage:** All memory states are lost when the simulator exits.
...
* **L2 Cache:** Larger but slower (5 + 1 cycle latency). Checked if L1 misses.
